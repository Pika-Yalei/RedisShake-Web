...
1. 空数据目录首次启动时，执行器自动创建唯一管理员，账号为 `admin`，密码为 `RedisShake@123456`。
2. Web 显示登录页，用户使用默认凭据登录，并在设置页修改密码；启动命令只显示访问地址。
3. 数据库已存在管理员时保留现有账号和密码；单行约束保证不会创建第二个管理员。
4. 忘记密码使用独立重置流程，不重新初始化管理员。

默认 Web 仅监听本机；对外开放访问前应修改默认密码并限制访问范围。旧数据目录中固定用户名 `admin` 的管理员会自动迁移，原密码保持不变。

密码使用 Argon2id 哈希；会话采用不可预测的随机令牌，服务端保存摘要，浏览器使用 HttpOnly、SameSite=Strict Cookie。当前会话绝对时长为 7 天，不单独设置空闲超时；登录按执行器实例做每分钟失败 10 次限速，注销销毁会话，修改或重置密码后撤销所有旧会话。状态更改接口校验 CSRF 令牌。

“设置 → 修改密码”填写原密码与新密码；忘记密码通过本机维护子命令签发一次性重置码，再在 Web 设置新密码，不打印或恢复旧密码。修改和重置密码都不影响正在同步的任务。
...
- 首次启动打印 Web 访问地址；执行器自动创建默认管理员，浏览器展示登录页。
...
| 编号 | 操作与预期 |
| --- | --- |
| RT-01 | 全新数据目录自动创建唯一默认管理员；已有账号和修改后的密码在重启后保持不变。 |
...
| RT-11 | 密码修改、会话过期、重置和初始化不停止运行；未登录不能执行管理动作。 |
