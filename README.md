...
基于 [RedisShake v4.6.2](https://github.com/tair-opensource/RedisShake/releases/tag/v4.6.2) 的自部署 Redis 迁移管理界面。通过中文 Web 页面保存连接、配置规则、预检并启动单向全量迁移；全量完成后持续同步增量。前端使用原生 JavaScript，服务端使用 Go 与 SQLite，RedisShake 作为独立进程运行。
...
启动时在终端显示 Web 地址。`serve` 会启动常驻执行器，然后运行 Web 服务；关闭 Web 进程不会停止已有同步任务。重新执行 `./redis-shake-web serve` 即可接回执行器。要有意停止执行器及任务，先在 Web 停止任务，再运行 `./redis-shake-web shutdown`。默认数据目录为 `~/.redis-shake-web`，可给命令传入 `--data-dir`；若单独设置 `--socket-dir`，后续管理命令也要使用同一参数。
